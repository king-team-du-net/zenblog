...
use App\Interface\Uploads\Picture\ResponseFactory;
...
class ResizerPictureController extends AbstractController
{
    ...
    public function resizerPicture(Request $request, int $width, int $height, string $path): Response
    {
    ...
    #[Route(path: '/picture/convert/{path}', name: 'picture_jpg', methods: [Request::METHOD_GET], requirements: ['path' => '.+'])]
    public function convertPicture(Request $request, string $path): Response
    {
        $server = ServerFactory::create([
            'source' => $this->publicPath,
            'cache' => $this->cachePath,
